...
serialize-response: func [ r format /localnum ] [
    either not equal? first r 'err [
        switch/default format [ 
			...
			map  [ dbdata-to-map r ]
			raw  [ r ]
			csv  [ dbdata-to-csv r localnum ]
			html  [ dbdata-to-html r localnum ]
			xml  [ dbdata-to-xml r ]
		] 'err
	] [
		switch/default format [ 
			...
			raw  [ 'err ]
			csv  [ "err!" ]
			csv  [ "<table><tr><td>err!</td></tr></table>" ]
			xml  [ "<xml>err!</xml>" ]
		] 'err
	]
]
...
format-csv-value: func [ v localnum /local wrap enquote wrap? is-value? ] [
	wrap: func [ v ] [ join3 {"} v {"} ]
...
dbdata-to-csv: func [ dbdata localnum /local R ] [
	data: dbdata/1  cols: dbdata/2
	insert data cols
	either not empty? cols [
		R: copy ""
		while [ not tail? data ] [
			fcell: true
			foreach c cols [ 
				append R rejoin [ (either fcell [""][";"]) format-csv-value (first data) localnum ] 
				data: next data
				fcell: false
			]
			append R newline
...
dbdata-to-xml: func [ dbdata ] [
	...
	either not empty? cols [
		...
		while [ not tail? data ] [
			append R "^/<invoice>"
			foreach c cols [ 
				append R rejoin [  wrap c format-csv-value (first data) false ] 
				data: next data
				fcell: false
			]
			append R "</invoice>^/"
...
dbdata-to-html: func [ dbdata localnum /local R ] [
	data: dbdata/1  cols: dbdata/2
	insert data cols
	either not empty? cols [
		R: copy {<html><head><meta http-equiv="Content-Type" content="text/html; charset=utf-8"></meta></head>
		   	<body><style>td { border: 1px solid silver; }</style><table>}
		fcell: true
		while [ not tail? data ] [
			append R "<tr>"
			foreach c cols [ 
				append R rejoin [ "<td " (either fcell [ "bgcolor='silver' fontcolor='white'"][""]) ">" format-csv-value (first data) localnum "</td>" ] 
				data: next data
			]
			fcell: false
			append R "</tr>"
		]
		head join R "</table></body></html>"
	] [	"<table></table>" ]
]
...
any-none?: func [ v ] [ any [ none? v equal? v 'none ] ]

enquote: func [ v /sql ] [ 
	either any [ integer? v any-none? v decimal? v logic? v ]
		[ either any-none? v [ "null" ] [ v ] ]
		[ rejoin [ "'" (either sql [ replace/all copy v "'" "''" ] [ v ]) "'" ] ]
]
